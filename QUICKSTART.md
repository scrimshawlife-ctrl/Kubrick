...
Kubrick is a **standalone OpenClaw Agent Skill** on the permanent **`openclaw`** branch (v0.13.0). Continuity Forge, MCP servers, model APIs, and external generation providers remain optional.

The Hermes edition remains on [`main`](https://github.com/scrimshawlife-ctrl/Kubrick/tree/main). This branch carries the OpenClaw packaging by **Prabu**, external mutable state, portability checks, and the deliberately ported v0.13 operator surface.

## 1. Install (OpenClaw / `openclaw`)

```bash
openclaw skills install git:scrimshawlife-ctrl/Kubrick@openclaw --global
# or, from this branch clone:
./install.sh
```
...
Mutable runtime state defaults to `~/.openclaw/state/kubrick`. Override it with `KUBRICK_STATE_DIR`; do not write receipts or learned overlays into the installed package.

### Hermes install (upstream `main`)
...
OpenClaw details and credits: [`docs/OPENCLAW.md`](docs/OPENCLAW.md).
...
| `SKILL.md` | OpenClaw Agent Skill operating contract |
