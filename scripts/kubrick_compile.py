...
def fail(out,stage,reasons,diagnostic=None):
    ...
    print(json.dumps(final,indent=2)); raise SystemExit(1)


def validate(out,artifact,schema,label):
    report=out/f"schema-{label}.json"
    result=run([PY,str(ROOT/"scripts/validate_artifact.py"),"--artifact",str(artifact),"--schema",str(ROOT/schema),"--output",str(report)])
...
def main():
    ...
    p.add_argument("--mode",choices=["single-frame","scene","storyboard","diagnostic"],default="single-frame")
    p.add_argument("--storyboard-plan")
    p.add_argument("--provider",choices=["none","generic","grok-imagine"],default="none")
    ...
    if len(forms)<2:
        forms=[{"id":f"observed_{i+1}","kind":"motif","observed_form":str(v),"initial_state":"present","target_state":brief.get("desired_state_change","mutated"),"provenance_label":"OBSERVED","pattern_links":[selected]} for i,v in enumerate((brief.get("observable_evidence") or [])[:3])]
    ...
    graph_input=out/"graph-input.yaml"; write(graph_input,graph_spec)
    graph_path=out/"motif-graph.private.yaml"
    ...
    schema_reports={"graph":validate(out,graph_path,"schemas/motif-structure-graph.schema.yaml","graph")}
    ...
    artifacts={"retrieval":"retrieval-receipt.yaml","private_graph":graph_path.name,"structured_packet":structured_path.name,"structured_audit":"structured-anti-slop-report.json","audience":"audience-constraints.yaml","text_audit":"text-anti-slop-report.json"}
    storyboard_path=None
    if a.mode=="storyboard":
        if not a.storyboard_plan: fail(out,"storyboard",["STORYBOARD_PLAN_REQUIRED"])
        storyboard_path=out/"storyboard-symbolic-state.yaml"
        prop=run([PY,str(ROOT/"scripts/propagate_graph_state.py"),"--graph",str(graph_path),"--plan",a.storyboard_plan,"--output",str(storyboard_path)])
        if prop.returncode!=0: fail(out,"storyboard",["STORYBOARD_PROPAGATION_FAILED"],prop.stdout or prop.stderr)
        transition_path=out/"storyboard-transition-report.json"
        comp=run([PY,str(ROOT/"scripts/compare_frame_state.py"),"--storyboard",str(storyboard_path),"--output",str(transition_path)])
        if comp.returncode!=0: fail(out,"storyboard",["STORYBOARD_CONTINUITY_FAILED"],comp.stdout or comp.stderr)
        schema_reports["storyboard"]=validate(out,storyboard_path,"schemas/storyboard-symbolic-state.schema.yaml","storyboard")
        artifacts.update({"storyboard_state":storyboard_path.name,"storyboard_transitions":transition_path.name})

    if a.provider!="none":
        adapter_path=out/"model-adapter-packet.yaml"
        cmd=[PY,str(ROOT/"scripts/build_model_adapter_packet.py"),"--graph",str(graph_path),"--provider",a.provider,"--output",str(adapter_path)]
        ...
        schema_reports["adapter"]=validate(out,adapter_path,"schemas/model-adapter-packet.schema.yaml","adapter")
        artifacts["model_adapter"]=adapter_path.name
        if a.provider=="grok-imagine":
            grok_path=out/"grok-imagine-prompt-packet.yaml"
            grok=run([PY,str(ROOT/"scripts/adapt_grok_imagine.py"),"--packet",str(adapter_path),"--output",str(grok_path)])
