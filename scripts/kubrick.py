...
def main()->None:
    ...
    parser.add_argument("command",choices=sorted(COMMANDS)); parser.add_argument("args",nargs=argparse.REMAINDER); ns=parser.parse_args()
    result=subprocess.run([PY,str(ROOT/"scripts"/COMMANDS[ns.command]),*ns.args],cwd=ROOT)
