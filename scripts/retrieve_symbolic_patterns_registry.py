#!/usr/bin/env python3
"""Registry-aware Kubrick retrieval for the standalone OpenClaw skill."""
...
from kubrick_paths import ensure_state_dirs, state_paths
...
CACHE_DIR=state_paths()["registry_cache"]
...
def persist(receipt):
    ensure_state_dirs(); key=receipt["retrieval_receipt"]["request_hash"]; cache=CACHE_DIR/f"{key}.json"; logged=RECEIPT_DIR/f"receipt-registry-{key}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}.json"
